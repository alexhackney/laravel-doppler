...
return [
    ...
    */
    ...
    'token' => [
        /*
         ...
         | different Doppler projects needs. /etc/doppler/.token is the box credential:
         | written once at provision time, and outside the release directory so a switch
         | to atomic deploys does not wipe it.
         |
    ...
    'target' => [
        ...
        /*
         | Keep one .env.bak generation so a bad render can be rolled back by hand
    ...
    */
    ...
    |   - If config:cache is configured and FAILS, no later hook runs and the command
    |     exits non-zero. A cache built from a bad file bakes in blank secrets while the
